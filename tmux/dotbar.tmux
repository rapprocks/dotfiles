...
resolve_color() {
  local color="$1"
  # If the color looks like a tmux format string (e.g., #{@thm_base}), resolve it
  if [[ "$color" =~ ^\#\{.*\}$ ]]; then
    tmux display-message -p "$color" 2>/dev/null || echo "$color"
  else
    echo "$color"
  fi
}
...
# Colors
bg=$(resolve_color "$(get_tmux_option "@tmux-dotbar-bg" '#0B0E14')")
fg=$(resolve_color "$(get_tmux_option "@tmux-dotbar-fg" '#475266')")
fg_current=$(resolve_color "$(get_tmux_option "@tmux-dotbar-fg-current" '#BFBDB6')")
fg_session=$(resolve_color "$(get_tmux_option "@tmux-dotbar-fg-session" '#565B66')")
fg_prefix=$(resolve_color "$(get_tmux_option "@tmux-dotbar-fg-prefix" '#95E6CB')")
